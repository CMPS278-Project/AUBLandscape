...
const URI =
    "mongodb+srv://jamal:j1a2m3a4l5@cluster0.6vwvu.mongodb.net/firstDB?retryWrites=true&w=majority";
app.use(express.urlencoded({ extended: true }));
...
app.post("/delete_plant", (req, res) => {
    var name = req.body.name;
    Plant.findOneAndRemove({ name: name }, function (err, member) {
        res.render("admin_options");
    });
});
...
app.post("/FilterByIdentification", (req, res) => {
    ...
    console.log(
        "Searching By Identification for plants with :\n",
        callParameters
    );
...
app.get("/popular_plant_categories", (req, res) => {
    switch (req.query.category) {
        case undefined:
            res.render("popular_plant_categories");
            break;
    
        case "fragrant":
            Plant.find({Category: "Fragrant"})
                .then((result) => {
                    res.render("filtered_plants_res", {
                        filterBy: "Category",
                        plants: result,
                ...
                .catch((err) => {console.error("Error in filtering by category")})
                break;
        
        case "flowering":
            Plant.find({Category: "Flowering"})
                .then((result) => {
                    res.render("filtered_plants_res", {
                        filterBy: "Category",
                        plants: result,
